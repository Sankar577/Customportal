...
$invoiceID = "INV_" . mt_rand(1000, 9999);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($errors)) {
        ...
        if ($_REQUEST['action'] == "add") {
            $sql = "INSERT INTO custom_invoice_details(`order_id`,`invoice_id`) VALUES ('" . $orderID . "','" . $invoiceID . "')";
            // echo $sql;
            $res = $db_cms->insert_query($sql);
            if ($res) {
                echo "<script>window.location.href = 'invoice_management.php?message=Added Successfully';</script>";
            }
        }
        if ($_REQUEST['action'] == "edit") {
...
<div class="content-wrapper">
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <select id="orderID" name="orderID" class="form-control">
                                        ...
                                        foreach ($order_res as $order) {
                                            $selected = ($order['order_id'] == $order_id) ? 'selected' : ''; // Check if the value matches
                                ...
                                <div class="row">
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <input type="email" id="email" value="<?= $payment_res[0]['email'] ?>"
                                ...
                                <div class="row">
                                    <div class="form-group col-md-6">
                                        <label for="invoice_id">Invoice Numbe</label>
                                        <input type="invoice_id" id="invoice_id" name="invoice_id" class="form-control"
                                            value="<?= $invoiceID ?>" readonly>
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label for="date">Date</label>
                                        <input type="date" id="date" value="<?= $payment_res[0]['date'] ?>"
    ...
    <script>
        document.getElementById('orderID').addEventListener('change', function() {
            ...
            if (orderID) {
                fetch('invoice/get_invoice_details.php?orderID=' + orderID)
                    .then(response => response.json())
                    .then(data => {
                        if (data.success) {
                            // Populate other fields
                            document.getElementById('address').value = data.customer_name + '\n' + data.address;
                            ...
                            document.getElementById('finalTotal').value = parseFloat(data.final_total).toFixed(
                                2);
